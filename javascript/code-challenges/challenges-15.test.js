...
const detectTicTacToeWin = (board) => {
  ...
  let newArr2 = [];
  for (let i = 0; i < board.length; i++) {
    newArr2.push(board[0][i] + board[1][i] + board[2][i]);
  }
  let newArr3 = [];
  for (let i = 0; i < board.length; i++) {
    newArr3.push(board[0][0] + board[1][1] + board[2][2]);
  }
  let newArr4 = [];
  for (let i = 0; i < board.length; i++) {
    newArr4.push(board[0][2] + board[1][1] + board[2][0]);
  }
  if (newArr.includes('XXX') || newArr.includes('OOO') || newArr2.includes('XXX') || newArr2.includes('OOO') || newArr3.includes('XXX') || newArr3.includes('OOO') || newArr4.includes('XXX') || newArr4.includes('OOO')) {
    return true;
  }
  else {
    return false;
