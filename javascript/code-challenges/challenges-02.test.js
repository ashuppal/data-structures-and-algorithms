...
const { index, is } = require('cheerio/lib/api/traversing')
...
const raisedToTheThird = arr => {
  // Solution code here...
  const result = arr.map(num => Math.pow(num, 3))
  return result
}
...
const addOne = arr => {
  // Solution code here...
  const addResult = arr.map(addNum => addNum + 1)
  return addResult
}
...
const addQuestion = arr => {
  // Solution code here...
  const question = arr.map(addQ => addQ + '?')
  return question
}
...
const mapTwoToThe = arr => {
  // Solution code here...
  let resultMap = arr.map(num => Math.pow(2, num));
  return resultMap;
}
...
const charCode = arr => {
  // Solution code here...
  let charResult = arr.map(result => result.charCodeAt(index));
